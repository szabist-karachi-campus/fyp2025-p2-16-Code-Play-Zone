...
// const PuzzleGame2Variables = ({ navigation }) => {
...
//   const requiredBlocksOptions = ['score', '=', '10', 'print', '(score)'];
...
//             },
//             body: JSON.stringify({ progress_item: 'Puzzle Level 2 Completed' }),
//           });
...
//             navigation.navigate('PuzzleGame3AO');
//           } else {
...
//       Drag and arrange blocks to assign a value to a variable score and print it.
//       </Text>
...
//             {placedBlocks.length
//                 ? `You placed:\n${placedBlocks
//                 .map((block,index) => (block === 'print' && index!==0 ? `\n${block}` : block))
//             .join(' ')}`
...
// export default PuzzleGame2Variables;
...
const PuzzleGame2Variables = ({ navigation }) => {
  ...
  const requiredBlocks = ['score', '=', '10', 'print', '(score)'];
  ...
  const handleCheck = async () => {
    try {
      ...
      const isSolutionCorrect = placedBlocks.length === requiredBlocks.length &&
        placedBlocks.every((block, index) => block === requiredBlocks[index]);
      ...
      if (isSolutionCorrect) {
        ...
        const response = await fetch('http://10.0.2.2:8000/api/game/progress/add', {
          ...
          body: JSON.stringify({ progress_item: 'Puzzle Level 2 Completed', points }),
        });
  ...
  const handleSkip = () => {
      Alert.alert("Confirm Skip", "Do you want to skip this level or game?", [
        { text: "Cancel", style: "cancel" },
        { text: "Level", onPress: () => navigation.navigate("PuzzleGame3AO") },

...
// const PuzzleGame4CS = ({ navigation }) => {
...
//   const [dropZoneDimensions, setDropZoneDimensions] = useState(null);
  ...
  // const requiredBlocksOptions = ['if', 'score', '>10:', 'jump()', 'else:', 'walk()'];
...
//             },
//             body: JSON.stringify({ progress_item: 'Puzzle Level 4 Completed' }),
//           });
...
//             navigation.navigate('PuzzleGame5Loops');
//           } else {
...
//       <Text style={styles.header}>Conditional Statements</Text>
//       <Text style={styles.objective}>Objective</Text>
//       <Text style={styles.instruction}>
//         Drag and arrange blocks to create logic where a character jumps if the score is greater
//         than 10, otherwise walks.
//       </Text>
...
//             ? `You placed:\n${placedBlocks
//               .map((block, index) =>
//               (block === 'else:' || block === 'jump()' || block === 'walk()') && index !== 0
...
const PuzzleGame4CS = ({ navigation }) => {
    ...
    try {
      ...
      if (isSolutionCorrect) {
        ...
        const progressResponse = await fetch('http://10.0.2.2:8000/api/game/progress/add', {
          ...
          body: JSON.stringify({ progress_item: 'Puzzle Level 4 Completed', points }),
        });
  ...
  const handleSkip = () => {
    Alert.alert("Confirm Skip", "Do you want to skip this level or game?", [
      { text: "Cancel", style: "cancel" },
      { text: "Level", onPress: () => navigation.navigate("PuzzleGame5Loops") },

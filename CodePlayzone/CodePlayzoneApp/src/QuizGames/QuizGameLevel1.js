...
const questions = [
  ...
  },
  {
    ...
    options: ['Python', 'Microsoft Word', 'Google Chrome'],
    correctAnswer: 0,
  },
];

const QuizGameLevel1 = () => {
  ...
  const handleAnswer = async (questionIndex, optionIndex) => {
    ...
    if (allAnswered) {
      ...
      try {
        ...
        if (authToken) {
          const response = await fetch('http://10.0.2.2:8000/api/game/progress/add', {
            ...
            body: JSON.stringify({
              progress_item: 'Quiz Level 1 Completed',
              points: finalPoints,
            }),
          });
      ...
      Alert.alert(
        ...
        [{ text: 'Next Level', onPress: () => navigation.navigate('QuizGameLevel2') }]
      );
    }
  };
  ...
  const handleSkip = () => {
    Alert.alert(
      ...
      [
        ...
        { text: "Level", onPress: () => navigation.navigate("QuizGameLevel2") },
  ...
  return (
    <ImageBackground source={require('../../pictures/backgroundimage.png')} style={styles.backgroundImage}>
      <View style={styles.container}>
        ...
        <Text style={styles.levelTitle}>Level 1: Introduction to Programming</Text>

...
//       Alert.alert('Level 7 Completed', `Your score: ${score + (optionIndex === questions[questionIndex].correctAnswer ? 1 : 0)}`, [
//         { text: 'Next Level', onPress: () => navigation.navigate('QuizGameLevel8') }
//       ]);
//     }
//   };
...
//         </View>
//         <Text style={styles.levelTitle}>Level 8: Functions</Text>
  ...
  const handleAnswer = async (questionIndex, optionIndex) => {
    ...
    if (allAnswered) {
      ...
      try {
        ...
        if (authToken) {
          const response = await fetch('http://10.0.2.2:8000/api/game/progress/add', {
            ...
            body: JSON.stringify({
              progress_item: 'Quiz Level 8 Completed',
              points: finalPoints,
            }),
          });
      ...
      Alert.alert(
        ...
        [{ text: 'Complete Quiz', onPress: () => navigation.navigate('QuizGamesSuccess') }]
      );
    }
  };
  ...
  const handleSkip = () => {
    Alert.alert(
      ...
      [
        ...
        { text: "Level", onPress: () => navigation.navigate("QuizGamesSuccess") },
